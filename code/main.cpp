...
// Remove um personagem da fila (caso exista)
...
bool esta_na_lista(std::pair<int, bool> personagem){
  for(size_t i = 0; i < fila.size(); i++) {
    if(fila[i].first == personagem.first && fila[i].second == personagem.second){
      return true;
    }
  }
...
bool tem_deadlock(){
  bool sheldonCasal = esta_na_lista({0, true});
  bool leonardCasal = esta_na_lista({2, true});
  bool howardCasal = esta_na_lista({4, true});

  bool sheldonSozinhx = esta_na_lista({0, false}) || esta_na_lista({1, false});
  bool leonardSozinhx = esta_na_lista({2, false}) || esta_na_lista({3, false});
  bool howardSozinhx = esta_na_lista({4, false}) || esta_na_lista({5, false});

  if(sheldonCasal && leonardCasal && howardCasal){
    // Se todos os casais estão na fila
    return true;
  }else if(sheldonSozinhx && leonardSozinhx && howardSozinhx){
    // Se existe exatamente um membro de cada casal na fila
    return true;
  }
